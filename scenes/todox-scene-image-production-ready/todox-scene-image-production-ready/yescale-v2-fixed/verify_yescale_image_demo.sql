...
BEGIN
    ...
    SELECT count(*) INTO customer_id_not_nullable
    ...
    IF customer_id_not_nullable > 0 THEN
        RAISE EXCEPTION 'billing.token_wallets.customer_id must be nullable for system wallets.';
    END IF;
    ...
    IF missing_permissions > 0 THEN
        RAISE EXCEPTION 'Required AI billing permissions are missing. Count=%', missing_permissions;
    END IF;
    ...
    IF stale_pending > 0 THEN
        RAISE EXCEPTION 'Stale image billing reservations/reconciliation rows require manual handling before demo enable. Rows=%', stale_pending;
    END IF;
