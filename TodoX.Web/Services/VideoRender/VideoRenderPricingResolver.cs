...
public sealed record VideoRenderResolvedPrice(
    string ModelName,
    string? Mode,
    int? DurationSeconds,
...
public interface IVideoRenderPricingResolver
{
    VideoRenderResolvedPrice Resolve(
...
public sealed class VideoRenderPricingResolver : IVideoRenderPricingResolver
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    public VideoRenderResolvedPrice Resolve(
        ProviderOptionDto option,
        AiProviderCapabilityDto capability,
        RVideoVideoModelPolicyEntry policy,
        string aspectRatio,
        ...
        int durationSeconds)
    {
        var model = (policy.Model ?? option.ModelName ?? string.Empty).Trim();
        if (string.IsNullOrWhiteSpace(model))
        {
            throw new InvalidOperationException("Thiếu model video.");
        }
        ...
        if (matched is not null)
        {
            ...
            return new VideoRenderResolvedPrice(
                model,
                mode,
                durationSeconds,
                chargedPoints,
    ...
    private static string BuildRuleKey(string model, string? mode, int durationSeconds)
        => $"{model}|{mode ?? "default"}|{durationSeconds}";
    ...
    private sealed record PricingMatch(
        string? Model,
        string? Mode,
        int? DurationSeconds);
}
