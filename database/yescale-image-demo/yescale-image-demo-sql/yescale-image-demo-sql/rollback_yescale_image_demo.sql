-- Rollback YEScale image demo defaults without deleting usage, wallet, or reconciliation history.
-- Requires snapshot written by 04_enable_yescale_image_demo.sql.
...
BEGIN
    ...
    SELECT count(*) INTO snapshot_count
     ...
     WHERE snapshot_key = 'yescale_image_demo_before'
...
UPDATE public.todox_ai_provider_capability c
   SET is_default = false,
       updated_by = 'manual_sql_demo_rollback',
       updated_at = now()
  FROM caps
...
UPDATE public.todox_ai_provider_capability c
 ...
 WHERE s.snapshot_key = 'yescale_image_demo_before'
