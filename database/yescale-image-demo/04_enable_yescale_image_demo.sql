-- Demo default switch for YEScale image capabilities.
...
CREATE TABLE IF NOT EXISTS billing.yescale_image_default_snapshot (
    ...
    captured_by text NOT NULL DEFAULT 'manual_sql_demo',
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.todox_ai_provider WHERE provider_code='yescale_task_image') THEN
        RAISE EXCEPTION 'Refusing demo enable: yescale_task_image provider is missing.';
    END IF;
    IF to_regclass('billing.ai_image_billing_records') IS NULL THEN
        RAISE EXCEPTION 'Run 01_add_or_update_billing_support.sql before demo enable.';
    END IF;
    ...
    IF missing_columns > 0 THEN
        RAISE EXCEPTION 'Run updated 01_add_or_update_billing_support.sql before demo enable. Missing reconciliation/tariff columns=%', missing_columns;
    END IF;
    ...
    IF missing_system_wallet <> 1 THEN
        RAISE EXCEPTION 'Refusing demo enable: TODOX_AI_IMAGE_SYSTEM wallet missing or duplicated. Count=%.', missing_system_wallet;
    END IF;
    ...
    IF stale_reconciliation > 0 THEN
        RAISE EXCEPTION 'Refusing demo enable: unresolved image billing reconciliation rows exist. Rows=%.', stale_reconciliation;
    END IF;
    ...
    LOOP
        ...
        IF row_count <> 3 THEN
            RAISE EXCEPTION 'Refusing demo enable: capability % must have nano-banana-2, gpt-image, seedream-5. Found %.', cap, row_count;
        END IF;
    END LOOP;
    ...
    IF bad_count > 0 THEN
        RAISE EXCEPTION 'Refusing demo enable: YEScale image capability has unit_cost_points <= 0. Rows=%', bad_count;
    END IF;
    ...
    IF default_count <> 6 THEN
        RAISE EXCEPTION 'Refusing demo enable: expected 6 nano-banana-2 billed routed default candidate rows, found %.', default_count;
    END IF;
END $$;
...
SELECT 'yescale_image_demo_before',
...
UPDATE public.todox_ai_provider
   SET enabled = true, updated_by = 'manual_sql_demo', updated_at = now()
 WHERE provider_code = 'yescale_task_image';
...
UPDATE public.todox_ai_provider_capability c
   SET is_default = false, updated_by = 'manual_sql_demo', updated_at = now()
  FROM caps
...
BEGIN
    ...
    IF bad_defaults > 0 THEN
        RAISE EXCEPTION 'Refusing demo enable: each image capability must have exactly one default. Bad groups=%', bad_defaults;
    END IF;
    ...
    IF bad_default_model > 0 THEN
        RAISE EXCEPTION 'Refusing demo enable: default model must be YEScale nano-banana-2. Rows=%', bad_default_model;
    END IF;
    ...
    IF bad_backup_selectable > 0 THEN
        RAISE EXCEPTION 'Refusing demo enable: seedream-5 backup must not be user-selectable. Rows=%', bad_backup_selectable;
    END IF;
END $$;
